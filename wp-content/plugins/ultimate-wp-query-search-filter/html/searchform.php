...
$html.="<div style='background-color:#ebebeb'>";

if(isset($options[0]['strchk']) && ($options[0]['strchk'] == '1') && $text_position =="bottom"){
		$stext  = '<div style="float:left;" class="'.$defaultclass.' '.$divclass.'">';
		//$stext .= '<label class="keywordLabel '.$defaultclass.' '.$divclass.'-keyword">'.$options[0]['strlabel'].'</label>';
		$oldvalue = (isset($_GET['skeyword'])) ? $_GET['skeyword'] : '';
		$stext .= '<input id="'.$divid.'_key" type="text" name="skeyword" style="height:50px;font-size:24px;" class="uwpqsftext" value="'.$oldvalue.'" />';
        $stext .= '</div>';
        $textsearch =  apply_filters('uwpqsf_string_search',$stext, $id,$divid,$defaultclass,$divclass,$options);
        $html .= $textsearch;
}
...
$wrapper .= '<div style="text-align:center;margin:20px 0;float:left;width:150px;">';
$wrapper .='<input class="submitbutton" value="" style="display:inline-block; width: 160px; height: 41px; margin: 5px 0 0 50px; padding: 0" type="'.$btype.'" value="'.$options[0]['button'].'" alt="[Submit]"  />';
$wrapper .='</div>';
