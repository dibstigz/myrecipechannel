		...
		$fields = array(
			...
			'general'	=> 	array(
				...
				'post_type' => array(
					...
					'type'			=>	'dropdown',
					'options'		=>	$this->get_new_data_source( array( __CLASS__, 'get_post_types' ), $source_id ),
				),
				#== Post Status =====
				'post_status' => array(
...
#== META BOX RENDERERS =================================================================================
	...
	public function render_word_trimming_metabox() { $this->render_metabox('word_trimming');  }
		...
		<script type="text/javascript">
			(function($){
				$(document).ready( function(){
					...
					var wprss_ftp_must_have_ft_image = $( '#<?php echo self::META_PREFIX; ?>must_have_ft_image' );
					...
					var wprss_ftp_featured_image_checked = function(){
						...
						wprss_ftp_must_have_ft_image.parent().toggleClass( 'wprss-ftp-inactive-meta', is_unchecked ).prev().toggleClass( 'wprss-ftp-inactive-meta', is_unchecked );
					}
	...
	public function post_thumbnail_meta_box() {
		...
		(function($){
			$(document).ready(function(){
				...
				$('#postimagediv > h3.hndle > span').text( "<?php _e( 'Fallback Featured Image' ); ?>" );
			});
		})(jQuery);
