...
final class WPRSS_FTP_Images {
	...
	public function get_cache() {
		if ( is_null( $this->_cache ) ) {
			$this->_cache = new WPRSS_Image_Cache();
		...
		if ( stripos( $url, "fbcdn" ) > 0 ){
			...
			if ( !is_wp_error( $response ) && isset( $response['body'] ) ) {
				...
				if ( isset( $json['images'] ) && isset( $json['images'][0] ) ) {
					...
					wprss_log("Using FB image {$hires_url} from 'images' instead of {$url}", NULL, WPRSS_LOG_LEVEL_SYSTEM);
					return $hires_url;
					...
					wprss_log("Using FB image {$hires_url} from 'format' at $idx instead of {$url}", NULL, WPRSS_LOG_LEVEL_SYSTEM);
					return $hires_url;
				} else {
					wprss_log('No images array found in FB Graph lookup', NULL, WPRSS_LOG_LEVEL_SYSTEM);
				}
			} else {
				wprss_log("Error when contacting FB Graph at {$graph_url}", NULL, WPRSS_LOG_LEVEL_ERROR);
			}
